...
+ 而**定点（Q‑format）** 通过固定小数位把实数变为整数。
  ...
  ![FPGA Stimulus](assets/logo.png)

  一个用于生成、导出、导入 FPGA/数字信号测试数据的工具集合（包含 Python GUI 与 MATLAB 脚本）。

  主要功能
  - 生成常用测试信号：正弦（Sine）、方波（Square）、伪随机序列（PRBS）等。
  - 支持定点量化（Signed/Unsigned、Q-format）、饱和/四舍五入策略。
  - 导出量化数据：`.hex` / `.mem` / `.bin`，以及原始浮点数据：`.csv` / `.mat` / `.npz`。
  - 导入并还原样本，支持根据元数据无歧义还原浮点信号。

  目录结构（简要）
  - `signal_generator_gui.py` — Python GUI（当前仓库主要交互程序）。
  - `signal_generator_gui.m` — MATLAB GUI 版本（如需 Matlab 支持请查看）。
  - `README_GUI_ForPy.md`、`README_GUI_ForMatlab.md` — 各平台更详细的使用说明。
  - `assets/` — 截图、示例数据等资源。
  ...
  2. 安装依赖（若使用 GUI 建议安装 `matplotlib`）：
  ...
  导出/导入说明
  - 导出：在 GUI 中，可选择导出量化数据（`hex`/`mem`/`bin`）或原始浮点（`csv`/`mat`/`npz`）。`.mat` 的支持会尝试使用 `scipy`；若未安装则回退到 `.npz`。
  - 导入：新增 `Import type`（Quantized / Raw）选项以匹配导出行为：
    - Quantized：支持 `.hex` / `.mem` / `.bin`，需要 `Total bits`、`Fractional bits`、`Signed/Unsigned` 等参数来正确还原。
    - Raw：支持 `.csv` / `.mat` / `.npz`，会读取浮点样本并在预览中展示（对 `.mat` 尝试使用 `scipy.io`，回退到 `numpy`）。

  示例：导出 48000 Hz、24-bit 的正弦量化文件为 `sine.hex`，或导出原始浮点为 `sine.csv`。

  设计与约定
  - 推荐使用 JSON 元数据与样本位图一起存放（字段示例：`numeric_type`, `sign`, `Nbits`, `frac`, `bytes_per_sample`, `endianness`, `fs`, `vmin`, `vmax`）。
  - 量化策略推荐：`round-to-nearest` + `saturate`。

  贡献
  - 欢迎提交 issue 与 PR。请保持变更小而明确，并在 PR 描述中说明测试步骤。

  许可
  - 本项目遵循 MIT 许可证（或根据项目实际许可修改）。

  更多资料
  - 若需深入了解定点编码规则、补码行为、字节序与样本布局，请参阅仓库内的附录文档或联系项目维护者。

  ---

  如果你需要我把 README 翻译成英文版本或添加 CI/Badge（例如 GitHub Actions、PyPI badge），我可以继续补充。
