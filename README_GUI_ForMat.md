...
2. **环境配置**：
   ...
   - **SysID Paths (P & S)**：加载初级通路 P(z) 和次级通路 S(z) 的脉冲响应（支持 `.mat` 文件，由系统搭配的 `LMS_SYS_ID.m` 系统辨识工具生成）。
   - **Extended Paths (Pri→Ref & Sec→Ref)**：
     - **Pri->Ref path**（可选）：设置源信号到参考传感器的传递路径。如果不指定，默认为 1（直通），即参考信号 $x(n)$ 等于源信号 $r(n)$。
     - **Sec->Ref path**（可选）：设置声反馈路径（从次级控制输出到参考传感器）。如果不指定，默认为 0（无反馈）。若设置此路径，系统会模拟声反馈效应对参考信号 $x(n)$ 的影响。
3. **参数调整**：
